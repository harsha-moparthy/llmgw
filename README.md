...
The next twelve were found by an adversarial audit pass (parallel reviewers over money, concurrency, streaming, the HTTP surface, and the README itself, each required to demonstrate a failure with a running test rather than assert one). Bugs 5-10, 13-15 are pinned by regression tests that I verified fail when the fix is reverted. Bugs 11, 12 and 16 are fixed but covered only by the existing suite, not by a dedicated test — stated rather than implied.
...
**15. Any authenticated tenant could exhaust memory through the `model` field.** The request's `model` string was used verbatim as a Prometheus label on the 404 path, and a label is a permanently retained series. So a tenant sending a fresh random model name per request minted unbounded series — an unbounded memory leak reachable from a request body, with a `404` as its only visible symptom. The `metrics` package documents "cardinality is bounded by configuration" as its own rule; this was the one place client input reached a label and the rule was not enforced. Fixed by collapsing any non-configured model to `"unknown"` inside the recording helper, so no call site can reintroduce it. A 200-request probe went from 5,200 leaked label occurrences to zero.

**16. `/metrics` exposed every tenant's spend, unauthenticated.** Prometheus endpoints are conventionally left open because they conventionally carry nothing sensitive. This one carries per-tenant cost, token volume and remaining budget — one customer's commercial data. Now gated behind `LLMGW_OPERATOR_TOKEN` when that is set. The default stays open deliberately: refusing to serve metrics without a token would silently break every existing scrape on upgrade, so the safe-by-default choice here is to make the gate available and say plainly that a deployment exposing this port beyond its monitoring network must set it.
